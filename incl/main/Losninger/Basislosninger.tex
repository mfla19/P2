...
\end{defn}
Det vil sige at en basisløsning ligger ikke inden for løsningsmængden, da der kun er $n$ krav opfyldt, der vil derfor være mindst $1$ krav der ikke er opfyldt og ud fra dette vil problemet ikke være løst.
...
Hvis $\vec{x}^*$ er en basisløsning, kaldes indgangene $x^*_{B(1)},\dots ,x^*_{B(m)}$ for basis variabler, hvor $x_i=0 \,\forall i\neq B(1),\dots,B(m)$ kaldes for ikke-basis variabler.\\ %måske et andet ord
